...
yearly_poisson_bootstrap <- function(monthly_pred, n_bootstrap = 1000, confidence_level = 0.95) {
  # Inputs:
  # monthly_pred: A vector of Poisson predictions for each month (length = 12).
  # n_bootstrap: Number of bootstrap samples (default = 1000).
  # confidence_level: Desired confidence level (default = 0.95).
  
  # Outputs:
  # yearly_mean: Sum of the 12-month predictions (yearly prediction).
  # yearly_lower: Lower bound of the aggregated yearly confidence interval.
  # yearly_upper: Upper bound of the aggregated yearly confidence interval.
  
  # Generate bootstrap samples
  yearly_bootstrap <- numeric(n_bootstrap)  # Storage for yearly bootstrap samples
  
  for (b in 1:n_bootstrap) {
    # Simulate monthly Poisson values for this bootstrap iteration
    bootstrap_sample <- rpois(length(monthly_pred), lambda = monthly_pred)
    # Aggregate into yearly total
    yearly_bootstrap[b] <- sum(bootstrap_sample)
  }
  
  # Compute yearly mean prediction
  yearly_mean <- sum(monthly_pred)
  
  # Compute confidence intervals from bootstrap samples
  lower_percentile <- (1 - confidence_level) / 2
  upper_percentile <- 1 - lower_percentile
  
  yearly_lower <- quantile(yearly_bootstrap, probs = lower_percentile)
  yearly_upper <- quantile(yearly_bootstrap, probs = upper_percentile)
  
  # Return results as a list
  return(list(
    yearly_mean = yearly_mean,
    yearly_lower = yearly_lower,
    yearly_upper = yearly_upper
  ))
}
...
for (i in seq(1, out.l, by = 12)) {
  ...
  for (model_idx in 1:15) {
    ...
    if (model_idx <= 3) {
      ...
      yearly_forecast<-predict(current_model, newxreg=current_xreg, n.ahead = 12, level = 0.9, global = TRUE, B = 2000)$pred 
      bt<-yearly_poisson_bootstrap(yearly_forecast, n_bootstrap = 1000, confidence_level = 0.95)
     
      yearly_forecast <- list(
       Forecast = bt$yearly_mean,
        Lower_CI = bt$yearly_lower,
        Upper_CI = bt$yearly_upper,
        Year = 1
      )
